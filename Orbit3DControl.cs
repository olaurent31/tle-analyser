...
// Type: TLE_Analyser.Orbit3DControl
...
{
  [ComVisible(true)]
  public class Orbit3DControl : UserControl
  {
    private static List<WeakReference> __ENCList = new List<WeakReference>();
    private WebView m_WebView;
    private EO.WebBrowser.Runtime m_EOPort;
    ...
    [DebuggerNonUserCode]
    static Orbit3DControl()
    {
    }

    public Orbit3DControl()
    {
      this.Load += new EventHandler(this.Orbit3DControl_Load);
      Orbit3DControl.__ENCAddToList((object) this);
      this.m_WebView = new WebView();
      this.m_EOPort = new EO.WebBrowser.Runtime();
    }

    [DebuggerNonUserCode]
    private static void __ENCAddToList(object value)
    {
      lock (Orbit3DControl.__ENCList)
      {
        if (Orbit3DControl.__ENCList.Count == Orbit3DControl.__ENCList.Capacity)
        {
          int index1 = 0;
          int num = checked (Orbit3DControl.__ENCList.Count - 1);
          int index2 = 0;
          while (index2 <= num)
          {
            if (Orbit3DControl.__ENCList[index2].IsAlive)
            {
              if (index2 != index1)
                Orbit3DControl.__ENCList[index1] = Orbit3DControl.__ENCList[index2];
              checked { ++index1; }
            }
            checked { ++index2; }
          }
          Orbit3DControl.__ENCList.RemoveRange(index1, checked (Orbit3DControl.__ENCList.Count - index1));
          Orbit3DControl.__ENCList.Capacity = Orbit3DControl.__ENCList.Count;
        }
        Orbit3DControl.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
      }
    }

    private void Orbit3DControl_Load(object sender, EventArgs e)
    {
      this.m_WebView.Create(MyProject.Forms.Home.orbit3dWeb.Handle);
      EO.WebBrowser.Runtime.RemoteDebugPort = 1234;
      this.m_WebView.Url = "C:/TLEANALYSER/test.html";
    }
